...
import urllib.parse
class StudentAddCoursesView(APIView):
    ...
    def get(self, request):
        ...
        filter_param = request.GET.get('filter', 'Tất cả')
        filter_param = urllib.parse.unquote(filter_param)  # Giải mã URL

        print(f"Filter param after decoding: {filter_param}")

        # Danh sách các title được chấp nhận để filter
        allowed_filters = ['a', 'b', 'c', 'd', 'e']

        # Lọc theo title
        if filter_param == 'Tất cả':
            print("Filter is 'Tất cả', fetching all courses.")
            courses = Course.objects.all()
        elif filter_param in allowed_filters:
            print(f"Filtering courses with exact title: {filter_param}")
            courses = Course.objects.filter(title__iexact=filter_param)
        else:
            print("Invalid filter, returning no courses.")
